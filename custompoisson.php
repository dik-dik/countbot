...
$title = $plustweets." Tweet";

if($plustweets!=1) {
	$title = $title."s";
}

$title =$title." in ".$silenthours. " hour";
...
foreach ($arr as $key => $value) {
	...
	$remaining_count = lookup(($value['days']+$silenthours/24),$model[$key]);
	$next_hour = $remaining_count - lookup(((($value['days']+$silenthours/24)+$silenthours/24)+1/24),$model[$key]);
	$next_six = $remaining_count - lookup(((($value['days']+$silenthours/24)+$silenthours/24)+6/24),$model[$key]);
	$next_twelve = $remaining_count - lookup(((($value['days']+$silenthours/24)+$silenthours/24)+12/24),$model[$key]);
	$next_day = $remaining_count - lookup(((($value['days']+$silenthours/24)+$silenthours/24)+1),$model[$key]);
	
	$value['nexttarget'] = $remaining_count+$nextcount;
	...
	echo "<div><a href='".$value['last_tweet_url']."'>Last tweet</a>: ".floor(getTotalHours($value['last_tweet_age'])).$value['last_tweet_age']->format(" h %i m")." ago</div>&nbsp;
			<br/>
		Target:&emsp;&emsp;&emsp;".number_format($value['nexttarget'],2).
